...
class FirebaseUserSaveRecipe {
  ...
  Future<Map<String, List<RecipeCardModelcuahuy>>> getmaprecipe() async {

    List<NtoNUserSavedRecipe> _list = await getListMyCategory();
      List<RecipeCardModelcuahuy> list_recipe = [];
     Map<String, List<RecipeCardModelcuahuy>> results = new HashMap();
    for (var mycategory in _list) {
      await FirebaseFirestore.instance
          .collection('recipecards')
          .where('recipeOwner',whereIn: mycategory.idRecipe)
          .get()
          .then((value) {
        value.docs.forEach((element) {
          list_recipe.add(RecipeCardModelcuahuy.fromMap(element.data()));
          print(element.data());
        });
